...
/**
* Bipartite Graph Matching function
*/
...
pair<double, vector<pair<pair<int, int>, double>>> calculate_graph_matching( 
	...
	double *etm) {
		...
		vector<pair<pair<int, int>, double>> non_exact_mathces;
        #ifdef VERBOSE
        // get matching data
        non_exact_mathces = m->get_non_exact_tokens();
        #endif
		return make_pair(similarity, non_exact_mathces);
}


/**
* Parallel Bounds Update for Refinement Phase
*/
				...
				*/
				...
				if (!iscandidate && !ispruned && sim >= *global_thetaK) {
					...
					*considered += 1;
					...
					std::set<int> candidate_tokens = (*sets)[x];
					std::set_intersection(query_tokens->begin(), query_tokens->end(), candidate_tokens.begin(), candidate_tokens.end(), 
										std::inserter(intersection, intersection.begin()));
					
					bucket_upperbound->initialize(x, intersection, candidate_tokens.size());
					lower_bounds->initialize_lower_bound(x, intersection);
					mtx->unlock();
					iscandidate = true;
				}
				// if a set is a candidate then : - we prune based on the upper bound first (if upperbound < thetaK)
				// 								  - else we prune based on lower bound, and udpate lower bound in the topkLB list
				if (iscandidate && tbest != tqbest) {
					mtx->lock();
					double UBx = bucket_upperbound->update(x, sim, (*sets)[x].size());
					mtx->unlock();

					if (UBx < *global_thetaK) {
						mtx->lock();
						*upperpruned += 1;
						candidate_sets->erase(x);
						pruned->insert(x);
						mtx->unlock();
					} else {
						mtx->lock();
						double LBx = lower_bounds->update(x, sim, tqbest, tbest, *query_tokens, (*sets)[x]);
						auto it = std::find_if(tkLB->begin(), tkLB->end(), [x](const pair<double, int>& p){ return p.second == x;});
						pair<double, int> new_lower_bound = make_pair(LBx, x);
						if (LBx == 1 && tkLB->rbegin()->first == 1) { 
							if (tkLB->rbegin()->second != x) {
								top_one_sets->insert(x);
							}
						} else if (LBx >= tkLB->begin()->first) {
							if (it != tkLB->end()) {
								tkLB->erase(it);
								tkLB->insert(new_lower_bound);
							} else {
								tkLB->insert(new_lower_bound);
...
/**
* Parallel Background Calculator for Post-Processing Phase
*/
class BackgroundCalculator {
	private:
		std::vector<int> being_computed;
		std::set<pair<double, int>, cmp_increasing> *tkUB;
		std::set<pair<double, int>, cmp_increasing> *tKdup;
		std::set<pair<double, int>, cmp_increasing> *tkLB;
	...
	public:
		...
		void post_processing_phase() {
			...
			while (!all_checked(tkUB, checked)) {
				if (target_set != -1) {
					...
					// auto it2 = std::find_if(tKdup->begin(), tKdup->end(), [target_set](const pair<double, int>& p){ return p.second == target_set;}); // find the target set in the topK-UB duplicate
					// tKdup->erase(it2); // remove this set from topK UB duplicate
					
					// break ties arbritarily based on scores
					// std::set<double> scores2;
					// auto tkUBit2 = tkUB->begin();
					// while (tkUBit2 != tkUB->end()) {
					// 	scores2.insert(tkUBit2->first);
					// 	tkUBit2++;
					// }
					
					// if (sim >= tkUB->begin()->first) {
					// 	if (std::find(scores2.begin(), scores2.end(), sim) == scores2.end()) {
					// 		tkUB->insert(make_pair(sim, target_set));
					// 	}
					// 	tKdup->insert(make_pair(sim, target_set));
					// }
					if (sim >= tkUB->begin()->first) {
						tkUB->insert(make_pair(sim, target_set));
						tKdup->insert(make_pair(sim, target_set));
					...
					while (tkUB->size() < k && upperbound_pq->size() > 0) {
						if (upperbound_pq->begin()->first >= tkLB->begin()->first) {
							...
							// added for tie breaking
							double check_score = top_pq.first;
							auto tkUBit = tkUB->begin();
							// std::set<double> scores;
							// while (tkUBit != tkUB->end()) {
							// 	scores.insert(tkUBit->first);
							// 	tkUBit++;
							// }
							// if (std::find(scores.begin(), scores.end(), check_score) == scores.end()) {
							// 	// cout << "Insert2 tk with score: " << check_score << endl;
							// 	tkUB->insert(top_pq);
		...
		void semantic_overlap(int target_set) {
			double LBx = lowerbounds->get(target_set, query_tokens, sets[target_set]);
			if (LBx >= tkUB->begin()->first) {
				mtx->lock();
				*early_pruned += 1;
				checked->at(target_set) = true;
				computed_exact->insert({target_set, LBx});
				mtx->unlock();
				cout << "early termination post processing" << endl;
			} else if (computed_exact->find(target_set) == computed_exact->end()) {
				...
				// mtx->lock();
				pair<double, vector<pair<pair<int, int>, double>>> data = calculate_graph_matching(qset, tset, validedge, tkLB, mtx, etm);
				double sim = data.first;
				mtx->lock();
				computed_exact->insert({target_set, sim});
				#ifdef VERBOSE
				non_exact_match_data->insert({target_set, data.second});
				#endif
				mtx->unlock();
		...
		BackgroundCalculator(
			...
			std::unordered_map<int, vector<pair<pair<int, int>, double>>> *non_em_data
		) {
			...
			etm = early_term;
			tKdup = tUB_dup;
			non_exact_match_data = non_em_data;
		}
};


/**
* Koios Algorithm for Semantic Overlap Search
*/
void koios(Environment *env, Database *ftdb, FaissIndexGpu *faissindex, string query, int k, int size, double alpha, std::set<int> operatable_sets,
						...
						int partition_number, vector<bool> *status_tracker, std::mutex *global_gmt, int *global_query_i, 
	...
	std::set<pair<double, int>, cmp_increasing> tkUB_duplicate; // Top-K UB
	std::unordered_map<int, vector<pair<pair<int, int>, double>>> non_exact_match_data;
	...
	// topK duplicate tracker
	std::unordered_map<double, vector<int>> topKduplicate;
	...
	for (auto it = query_tokens.begin(); it != query_tokens.end(); it++){
		...
		if (std::find(dictionary.begin(), dictionary.end(), tq) == dictionary.end()){
			...
			// words covered by pre-trained model
	...
	while (token_stream.size() > 0) {
		...
		// cout << "ThetaK: " << tkLB.begin()->first << endl;
		token_stream_tracker[tqbest] += 1;
		// cout << "Query token: " << env->toWord(tqbest) << " | Search Result: " << env->toWord(tbest) << " | Similarity is: " << sim << endl;
		
		double old_thetaK = tkLB.begin()->first;
		std::set<int> sets_considered = inverted_index[tbest];
		std::set<int> intersection;
		// operatable_sets.insert(query_i); // remove before final_run
		std::set_intersection(sets_considered.begin(), sets_considered.end(), operatable_sets.begin(), operatable_sets.end(), 
							std::inserter(intersection, intersection.begin()));
		std::vector<int> sets_considered_vector(intersection.begin(), intersection.end());
		...
		ParallelBoundsUpdate *pbu = new ParallelBoundsUpdate(sets_considered_vector, &candidate_sets, &pruned, &query_tokens, &sets, ftdb, tbest, tqbest,
																...
																&upperpruned, &top_one_LB_sets_found, &top_one_sets, global_thetaK);
		...
		top_one_LB_sets_found += top_one_sets.size();
		for (auto top_one_it = top_one_sets.begin(), next_it = top_one_it; next_it != top_one_sets.end(); top_one_it=next_it) {
			++next_it;
			// cout << "Pruned due to top_one" << endl;
			candidate_sets.erase(*top_one_it);
			pruned.insert(*top_one_it);
			top_one_sets.erase(top_one_it);
		}
		
		if (sim == alpha) {
			sim = 0.0;
		}
		vector<int> to_be_pruned = bucket_upperbound->prune(sim, tkLB.begin()->first);
		for (vector<int>::iterator it = to_be_pruned.begin(); it != to_be_pruned.end(); it++) {
			int elem = *it;
			if (elem ==  query_i) {
				// cout << "Pruned due to upperbound" << endl;
			}
			pruned.insert(elem);
			if (std::find(candidate_sets.begin(),candidate_sets.end(), elem) != candidate_sets.end()) {
				candidate_sets.erase(elem);
				upperpruned += 1;
			}
		}
	...
	for (auto it = candidate_sets.begin(); it != candidate_sets.end(); it++) {
		int set = *it;
		double UB = bucket_upperbound->get(set, 0.0);
		double LB = lowerbounds->get(set, query_tokens, sets[set]);
		if (UB >= tkLB.begin()->first) {
			upperbound_pq.insert(make_pair(UB, set));
	...
	BackgroundCalculator *bg_calc = new BackgroundCalculator(being_computed, &tkUB, &tkLB, &upperbound_pq, &computed_exact, &checked, query_tokens, sets, 
																...
																alpha, &early_pruned_post_processing, k, validedge, query_i, &early_termination_match, &tkUB_duplicate, &non_exact_match_data); 
	...
	#ifdef VERBOSE
	// ************************** Printing duplicate RESULTS ********************
	if (tkUB_duplicate.size() == 0) {
		cout << "No TKDUP" << endl;
	} else {
		cout << "TKDUP" << endl;
		while (tkUB_duplicate.size() > 0) {
			// cout.precision(17);
			double sim_score = tkUB_duplicate.begin()->first;
			if (sim_score == tkUB.begin()->first) {
				cout << env->getSetName(tkUB_duplicate.begin()->second) << " : " << tkUB_duplicate.begin()->first << endl;
			}
			tkUB_duplicate.erase(tkUB_duplicate.begin());
		}
		cout << "TKDUP Done" << endl;
	}
	// ************************** Printing NON EXACT MATCHES ********************
	vector<int> sets_in_final;
	auto nem_it = tkUB.begin();
	while (nem_it != tkUB.end()) {
		sets_in_final.push_back(nem_it->second);
		nem_it++;
	}
	cout << "Exact Matches calculated for: " << computed_exact.size() << endl;
	auto nem_it2 = non_exact_match_data.begin();
	while (nem_it2 != non_exact_match_data.end()) {
		int set_id = nem_it2->first;
		vector<pair<pair<int, int>, double>> non_exact_matches = nem_it2->second;
		if (std::find(sets_in_final.begin(), sets_in_final.end(), set_id) != sets_in_final.end()) {
			cout << "For Set: " << env->getSetName(set_id) << endl;
			double avg_sim = 0.0;
			for (auto nemd : non_exact_matches) {
				pair<int, int> data_pair = nemd.first;
				double sim = nemd.second;
				avg_sim += sim;
				cout << "Query token: " << env->toWord(data_pair.first) << " | Search Result: " << env->toWord(data_pair.second) << " | Similarity is: " << sim << endl;
			}
			cout << "Average Sim: " << avg_sim / non_exact_matches.size() << endl;
		}
		nem_it2++;
	}
	#endif // end VERBOSE
...
/**
* Algorithm for Syntactic Overlap Search
*/
void algorithm_pureoverlap(Environment *env, Database *ftdb, string query, int k, int size, double alpha, std::set<int> operatable_sets,
	...
	int query_i = env->getSetId(query); // get setID for the query
	std::set<int> query_tokens = sets[query_i];
	int query_card = query_tokens.size();
	std::set<pair<double, int>, cmp_increasing> tk; // Top-K LB
	std::set<pair<double, int>, cmp_increasing> tkLB; // Top-K LB
	std::set<pair<double, int>, cmp_increasing> tkLB_duplicate; // Top-K UB
	double res_time = 0.0;
	...
	while (tk.size() < k){
		tk.insert(make_pair(0.0, -1));
	}

	// iterate over sets, if set_id in operatable_sets,
	// calcualte the pure overlap and insert in tkLB.
	// if pure_overlap > tkLB.head() then insert and remove

	auto it = sets.begin();
	while (it != sets.end()) {
		int set_id = it->first;
		if (std::find(operatable_sets.begin(),operatable_sets.end(), set_id) != operatable_sets.end()) {
			std::set<int> candidate_tokens = it->second;
			std::set<int> intersection;
			...
			double pure_ovelap_sim =   intersection_size / query_card;

			double old_min_value = tk.begin()->first;
			if (pure_ovelap_sim >= old_min_value) {
				// for breaking ties on scores
				// std::set<double> scores;
				// auto tkit = tk.begin();
				// while (tkit != tk.end()) {
				// 	scores.insert(tkit->first);
				// 	tkit++;
				// }
				// if (std::find(scores.begin(), scores.end(), pure_ovelap_sim) == scores.end()) {
				// 	tk.insert(make_pair(pure_ovelap_sim, set_id));
				// 	tk.erase(tk.begin());
				// }
				tk.insert(make_pair(pure_ovelap_sim, set_id));
				tk.erase(tk.begin());
				tkLB_duplicate.insert(make_pair(pure_ovelap_sim, set_id));
			}
		}
		it++;
	}
	...
	preend = std::chrono::high_resolution_clock::now();
	prelapsed = preend - prestart;
	res_time = prelapsed.count();
	#ifdef VERBOSE
	if (tkLB_duplicate.size() == 0) {
		cout << "No TKDUP" << endl;
	} else {
		cout << "TKDUP" << endl;
		while (tkLB_duplicate.size() > 0) {
			// cout.precision(17);
			double sim_score = tkLB_duplicate.begin()->first;
			if (sim_score == tk.begin()->first) {
				cout << env->getSetName(tkLB_duplicate.begin()->second) << " : " << tkLB_duplicate.begin()->first << endl;
			}
			tkLB_duplicate.erase(tkLB_duplicate.begin());
		}
		cout << "TKDUP Done" << endl;
	}
	#endif 
	global_gmt->lock();
	while (tk.size() > 0) {
		std::pair<double, int> tk_set = *(tk.begin());
		finalresults->insert(tk_set);
		tk.erase(tk.begin());
	}
	status_tracker->at(partition_number) = true;
	*response_time = std::max(*response_time, res_time);
	global_gmt->unlock();
}
...
int main(int argc, char const *argv[]) {
	// arguments: datalake_location, query, result_folder, alpha, k, partitions, {1,0}(pure_overlap)
	string setloc = argv[1]; // datalake location
	string query = argv[2];
	string writeto = argv[3];
	double alpha = stod(argv[4]);
	int k = stoi(argv[5]);
	int number_of_partitions = stoi(argv[6]);
	...
	for (auto it = sets.begin(); it != sets.end(); it++) {
		...
		partitions[part_id].insert(it->first);
	}
	...
	// to keep track whether a thread has finished or not
	...
	string path = setloc + query;
	std::mutex gmtx_parallel;
	for (int i = 0; i < number_of_partitions; i++) {
		cout << "Starting Partition: " << i + 1 << endl;
		if (!pure_overlap) {
			parallel_algo_pool.push_back(std::async(koios, env, ftdb, faissindex, path, k, 10000000, alpha, partitions[i], 
													&considered, &upperpruned, &preprocessing_time, &postprocessing_time, &top_oneLB, &postprocessing_sets, &token_stream_size, 
													&final_results, i, &completed, &gmtx_parallel, &query_i, 
			...
			parallel_algo_pool.push_back(std::async(algorithm_pureoverlap, env, ftdb, path, k, 10000000, alpha, partitions[i], &final_results, i, &gmtx_parallel, &completed, &response_time_post));
		}
	}

	// join completed process
	int i = 0;
	while (std::accumulate(std::begin(completed), std::end(completed), 0) != completed.size()) {
		i += 1;
	}
	...
	if (pure_overlap) {
		...
		while (counter < k && final_rit != final_results.end()) {
			...
			set<int> tset = (*(sets.find(target_set))).second;
			pair<double, vector<pair<pair<int, int>, double>>> data = calculate_graph_matching(qset, tset, validedge, &tkLB, &p_mtx, &etm);
			double sim = data.first;
	...
	string writepath = writeto + query;
	ofstream OutFilew(writepath);
	cout << "Result for " << query << endl;
