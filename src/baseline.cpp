/*
* Baseline : Semantic Overlap C++ Implementation
* See LICENSE file 
*/
...
class ValidMatrix {
	...
	public:
		...
		double solveQ(int q, std::set<pair<double, int>, cmp_increasing> *L_, std::mutex *mtx, double *etm, bool isEarly = false){
		...
		pair<int, int> get_match_size_data() {
			int total_number_of_matches = 0;
			int non_exact_matches = non_exact_token_indices.size();
			for (auto a : assignment_internal) {
				if (a != -1) {
					total_number_of_matches++;
				}
			}
			int exact_matches = total_number_of_matches - non_exact_matches;
			return make_pair(exact_matches, non_exact_matches);
		}
};
...
class RefinedLowerBound {
	...
	public:
		...
		void set_lowerbound(int c_id, double new_sim) {
            if (candidate_set_si.find(c_id) != candidate_set_si.end()) {
			    candidate_set_si.at(c_id) = new_sim;
            } else {
                candidate_set_si.insert({c_id, new_sim});
            }
		}
};
...
pair<double, pair<int, int>> thread_match_size_data(
	set<int> query_tokens,
	set<int> target_tokens,
	unordered_map<size_t, double> validedge,
 	std::set<pair<double, int>, cmp_increasing> *tkLB,
	...
	double sim = 0.0;
	ValidMatrix *m = new ValidMatrix(query_tokens, target_tokens, validedge);
	sim = m->solveQ(query_tokens.size(), tkLB, mtx, etm);
	pair<int, int> match_data = m->get_match_size_data();
	return make_pair(sim, match_data);
}
...
/**
* Baseline Algorithm for Semantic Overlap Search
*/
void baseline(Environment *env, Database *ftdb, FaissIndexGpu *faissindex, string query, int k, int size, double alpha,
						double *global_considered, double *global_upperpruned, double *globalpretime, double *globalposttime, 
						double *globaltopone, double *globalpostsets, double *globaltokenstreamsize, std::set<pair<double, int>, cmp_decreasing> *finalresults, std::mutex *global_gmt, int *global_query_i, 
    ...
    std::set<pair<double, int>, cmp_increasing> final_tkUB; // Top-K UB
	// std::set<pair<double, int>, cmp_increasing> tkUB_duplicate; // Top-K UB

	auto rit = tkUB.rbegin();
	while (rit != tkUB.rend()) {
		int set_id = rit->second;
		double sim = rit->first;
		if (final_tkUB.size() < k) {
			final_tkUB.insert(make_pair(sim, set_id));
		}
		rit++;
	}
	...
	auto fit = tkUB.begin();
	double topk_score = final_tkUB.begin()->first;
	cout << "TKDUP" << endl;
	while (fit != tkUB.end()) {
		int set_id = fit->second;
		double sim_score = fit->first;
		if (sim_score == topk_score) {
			cout << env->getSetName(set_id) << " : " << sim_score << endl;
		}
		fit++;
	}
	...
	auto nem_it = final_tkUB.begin();
	while (nem_it != final_tkUB.end()) {
		sets_in_final.push_back(nem_it->second);
		nem_it++;
	}
	cout << "Exact Matches calculated for: " << computed_exact.size() << endl;
	for (auto target_set : sets_in_final) {
		set<int> qset = (*(sets.find(query_i))).second;
		set<int> tset = (*(sets.find(target_set))).second;
		double etm_internal = 0.0;
		pair<double, pair<int, int>> match_data = thread_match_size_data(qset, tset, validedge, &tkLB, &gmtx, &etm_internal);
		pair<int, int> size_data = match_data.second;
		cout << env->getSetName(target_set) << " | EM: " << size_data.first << " | NEM: " << size_data.second << endl;
	}
	...
	// ************************** MERGING RESULTS ******************************
	cout << "Pure Overlap Score" << endl;
	auto final_it = final_tkUB.rbegin();
	int query_card = query_tokens.size();
	while (final_it != final_tkUB.rend()) {
		int set_id = final_it->second;
	...
	cout << "Semantic Overlap Score" << endl;
	while (final_tkUB.size() > 0) {
		// cout.precision(17);
		cout << env->getSetName(final_tkUB.begin()->second) << " : " << final_tkUB.begin()->first << endl;
		std::pair<double, int> tkUB_set = *(final_tkUB.begin());
		global_gmt->lock();
		finalresults->insert(tkUB_set);
		global_gmt->unlock();
		final_tkUB.erase(final_tkUB.begin());
	}
...
int main(int argc, char const *argv[]){
	// arguments: datalake_location, query, result_folder, alpha, k
