...
/********************************
 ALGORITHM
 ********************************/
void algorithm(Environment *env, Database *ftdb, FaissIndexGpu *faissindex, string query, int k, int size, double alpha, std::set<int> operatable_sets,
...
int main(int argc, char const *argv[]){
	...
	// batch queries to save time building indexes
	int qnum = 1;
	for (vector<string>::iterator it = queries.begin(); it != queries.end(); it++) {
		...
		for (int i = 0; i < number_of_partitions; i++) {
			...
			parallel_algo_pool.push_back(std::async(algorithm, env, ftdb, faissindex, path, k, 10000000, alpha, partitions[i], 
